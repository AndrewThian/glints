...
function addLargeNumber(strNum1, strNum2) {
  ...
  for (let _i = len1 - 1; _i >= 0; _i--) {
    a = arr1[_i];
    b = arr2[_i];

    const sum = a + b + carry;

    const _splitNumber = splitNumber(sum);


    const _splitNumber2 = _slicedToArray(_splitNumber, 2);


    const tens = _splitNumber2[0];


    const ones = _splitNumber2[1];
